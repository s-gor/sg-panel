# SG-Panel v0.9.4 — EC2 Ready

SG-Panel — небольшая панель управления Xray Reality на Python, Flask и
SQLite.

Версия 0.9.4 сохраняет все функции и дизайн v0.9.2, но добавляет полноценный
...
Версия 0.9.4 предназначена для контрольной чистой установки на EC2. После
проверки установки, перезагрузки, сертификата и подключения клиента сборка
будет зафиксирована как стабильная версия 1.0.0.

## Чистое удаление

Для повторной проверки установки SG-Panel можно удалить панель и Xray, сохранив
сертификаты Let's Encrypt, Nginx, Certbot, системные пакеты и swap:

```bash
sudo bash /opt/xpanel-mvp/deploy/uninstall.sh --yes
```

После удаления сервер готов к повторному запуску `install-from-github.sh`.
