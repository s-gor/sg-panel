...
<p align="center">
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-584%20tests-35d69a">
</p>
...
> Сборка интерфейса: **Preview 9 · FIX40 · UI23**. В Hysteria2 добавлено управление Salamander FinalMask с проверкой Xray, candidate/rollback, едиными ссылками/QR/subscriptions и сохранением пароля в backup. Реальное клиентское подключение Salamander на EC2 требует отдельной приёмки.

> Текущий восстановленный исходный кандидат: **UI23 Repair 1**. Он собран от UI22 с полным набором тестовых файлов и исправляет совместимость миграции Salamander. Это ещё не подтверждённый полный установщик и не опубликованная GitHub-база.

## Что такое SG-Panel

SG-Panel разворачивает и обслуживает собственный Xray-сервер на Ubuntu 22.04 и новее. Панель управляет клиентами, входящими профилями, подписками, трафиком, маршрутами, Outbounds, DNS, Cloudflare WARP, HTTPS, резервными копиями и обновлениями. Multi-Node объединяет Cluster Controller и удалённые SG-Node. Controller является источником истины для клиентских записей и хранит отдельные deployments на Controller и SG-Node. Обычный интерфейс показывает только сервер, профиль и состояние, а порядок выдачи подключений выбирается автоматически. Ручное управление порядком скрыто в `Clients → карточка клиента → Серверы подключения → Эксперт · Управление подключениями`.
...
## Cluster: Controller, SG-Node и центральная база клиентов

Раздел **Cluster** показывает Controller и SG-Node, состояние Agent/Worker и покрытие центральной клиентской базы. Каждый клиент создаётся один раз на Controller. Для каждого deployment обычный интерфейс показывает только:

- сервер, на котором клиент развёрнут;
- профиль подключения;
- фактическое состояние применения;
- включение подключения в постоянную подписку.

По умолчанию Controller сам определяет порядок выдачи проверенных подключений. Ручной порядок доступен только внутри закрытого экспертного блока.

Для нового SG-Node сохраняется проверенный порядок подключения:

1. **Подготовить чистую Ubuntu как SG-Node** — установить Agent, Worker и runtime.
2. **Добавить подготовленную SG-Node** — создать карточку и одноразовый enrollment token.
3. **Подключить установленную SG-Node** — выполнить команду на сервере Node и дождаться heartbeat.
4. **Проверить статус** — Agent, Worker, адрес, версии и состояние «В сети».
5. **Развернуть профиль** — подготовить клиента или сразу центральную клиентскую базу на Node.

Массовое развёртывание центральной клиентской базы формирует одно атомарное задание Agent, сохраняет существующих клиентов из последней подтверждённой конфигурации Node, выполняет `xray run -test`, создаёт страховочную копию, применяет конфигурацию и выполняет rollback при ошибке. Новое подключение появляется в стабильной подписке только после успешной проверки.

Постоянная подписка может одновременно выдавать Controller и подтверждённые SG-Node. В обычном режиме Controller сам выбирает порядок подключений; после обновления подписки клиентское приложение получает актуальный состав и порядок. **Одиночная VLESS-ссылка, уже импортированная в стороннее приложение, сама по себе измениться не может**: для неё требуется новая ссылка, стабильный совместимый домен либо постоянная подписка.

Подключение уже установленной полной SG-Panel в качестве Node зафиксировано как отдельная задача совместимости и в Preview 8 не заявляется исправленным. Текущая команда onboarding рассчитана на подготовленный SG-Node runtime.

Подробнее: [Cluster, Multi-Node и SG-Node Agent](docs/MULTI-NODE.md).

### Центральная клиентская база и deployments

Controller хранит постоянную запись клиента — имя, UUID/Auth, срок, комментарий и subscription token — отдельно от серверных deployments. Это позволяет:

- развернуть одного клиента на нескольких серверах без создания нового UUID;
- видеть в `Clients` сервер, профиль и фактическое состояние каждого deployment;
- включать или исключать отдельный endpoint из постоянной подписки;
- массово развернуть центральную базу на выбранной Node;
- добавлять проверенную Node в стабильную подписку только после успешного Agent-задания;
- сохранить историю массовых операций и их результат;
- включить deployments и операции переноса в обычную SQLite-резервную копию Controller.

Preview 8 — безопасная основа, а не завершённая автоматика катастрофоустойчивости. Автоматический health-based failover, агрегация удалённого трафика и универсальная миграция между любыми транспортами остаются следующими этапами.
...
- обычный экран показывает фактический XHTTP mode, установленную версию Xray, `XMUX: автоматически` и `Padding: стандартный Xray`;
- для XMUX доступны режимы «Автоматически», «Сниженное число соединений» и «Эксперт»; конфликт `maxConnections` с `maxConcurrency` блокируется до сохранения;
- точные XMUX-лимиты, диапазоны и Server/Client Extra находятся только внутри закрытого блока «Эксперт»;
...
- GeoFiles показывают фактический источник, пути, размеры, даты, SHA-256, семейство и число категорий активной пары `geoip.dat` / `geosite.dat`;
- доступны комплект SG Client, Loyalsoldier, RunetFreedom, RoscomVPN, пользовательские HTTPS URL и загрузка либо выбор локальных файлов;
- отсутствие `category-ads-all` само по себе не считается повреждением: структурная исправность файлов и совместимость с текущим Routing проверяются отдельно.

Оба раздела используют явное включение редактирования, обязательную проверку, `xray run -test`, backup и подтверждённый rollback. GeoFiles никогда не удаляют отсутствующие ссылки и не отключают пользовательские правила автоматически: несовместимость блокирует применение до ручного исправления либо выбора совместимого серверного preset. Полный будущий Routing и Xray config тестируются со staging до изменения live-файлов; Controller и каждая SG-Node хранят собственное поколение GeoFiles.
...
- до трёх `VLESS REALITY` точек с нейтральными именами `Профиль 1`, `Профиль 2`, `Профиль 3`;
...
### Clients и центральные deployments
...
- подключения на Controller и SG-Node без технических ролей в обычных названиях;
- персональное и массовое развёртывание через SG-Node Agent;
- последняя активность и online-состояние Controller;
- текущая скорость, сессия, день, месяц и общий трафик Controller;
- график за 14 дней;
- персональный и общий сброс статистики без удаления профилей.

Трафик удалённых SG-Node пока не агрегируется в общую историю: интерфейс честно отделяет локальную статистику Controller от состояния deployments.
...
- Ubuntu 22.04 или новее, включая промежуточные и будущие выпуски;
...
Загрузите на чистую Ubuntu 22.04 или новее один самодостаточный файл:
