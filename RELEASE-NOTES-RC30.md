...
RC30 объединяет установку по IP, пять рабочих Inbound-профилей, Clients & Traffic Studio, долговременную статистику и безопасный fallback обычного HTTPS на локальный Nginx.
...
- для XHTTP/gRPC TLS Nginx обслуживает обычный HTTPS и передаёт только клиентский path/service;
