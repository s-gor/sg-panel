...
# SG-PANEL CAP3 REPAIR PRESERVE OLD CHECKS
validate_result(){
  ...
  for (( attempt=1; attempt<=max_attempts; attempt++ )); do
    ...
    printf '[SG-Panel] Финальная проверка %s/%s: службы или панель ещё не готовы\n' \
      "$attempt" "$max_attempts"

    (( attempt < max_attempts )) && sleep 1
  done

  printf '[SG-Panel] Финальная проверка не пройдена после %s попыток\n' \
    "$max_attempts" >&2
  return "$rc"
}
