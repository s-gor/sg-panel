...
def test_rc42_docs_describe_saved_links_and_live_installer():
    ...
    for value in (readme, clients):
        assert not re.search(r"\b(?:Primary|Backup|Alt)\b", value)
        assert "автомат" in value.lower() or "Профиль 1" in value
    assert "Primary" in release and "Backup" in release and "Alt" in release
