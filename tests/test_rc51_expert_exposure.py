...
def test_rc52_advanced_is_contextual_and_read_only_by_default(panel):
    app = panel
    ...
    assert "Включить редактирование" in advanced_body
    assert "data-expert-edit-fieldset disabled" in advanced_body
    assert "Выключено — рекомендуется" in advanced_body
    assert "Vision настраивается в Inbound" in advanced_body
    assert "TLS certificate extras" in advanced_body
    assert 'name="ech_mode"' not in advanced_body
    ...
    assert "Installed Xray" in geofiles_body or "Комплект установленного Xray" in geofiles_body
