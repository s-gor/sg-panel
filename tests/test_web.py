...
class V05WebTest(WebTest):
    def test_new_admin_pages(self):
        self.login()
        for path, marker in (
            ("/settings", "Основное подключение".encode("utf-8")),
...
class V09SecurityWebTest(WebTest):
    ...
    def test_panel_access_job_can_be_started(self):
        ...
        self.assertNotIn(b"window.open", job_page.data)
        self.assertIn(b'target="_self"', job_page.data)
        self.assertIn("в этой же вкладке".encode("utf-8"), job_page.data)
...
class V095OutboundWebTest(WebTest):
    def test_outbound_page_explains_supported_combinations(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn("VLESS REALITY".encode("utf-8"), response.data)
...
class RC5ProfilesAndWarpWebTest(WebTest):
    def test_settings_shows_supported_profiles_without_deprecated_grpc_card(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"VLESS REALITY", response.data)
        self.assertIn(b"VLESS XHTTP-TLS", response.data)
        self.assertIn(b"VLESS XHTTP-REALITY", response.data)
        self.assertIn(b"Hysteria 2", response.data)
        self.assertIn("Nginx принимает TLS. До трёх XHTTP Path на одном публичном TCP-порту.".encode("utf-8"), response.data)
