...
def test_selected_profile_explains_connection_path_before_settings():
    ...
    assert 'id="inbound-route-client"' in html
    assert 'id="inbound-route-security"' in html
    assert 'id="inbound-route-server"' in html
    assert html.index('id="inbound-route-client"') < html.index("Параметры подключения клиента")
    assert "В клиентской ссылке" in html
    assert "На сервере" in html
    assert "Черновик — ещё не применён" in html
