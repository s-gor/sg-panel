...
def test_rc50_tables_are_initialized(panel):
    ...
    assert geofiles["selected_source"] == "xray"
    with connect() as con:
        tables = {
            row[0]
...
def test_supported_geofiles_sources_and_url_rules(panel):
    for source in ("v2fly", "loyalsoldier", "runetfreedom"):
...
def test_local_geofiles_check_and_apply_is_atomic(
    ...
    source_geoip.write_bytes(b"I" * 4096)
    source_geosite.write_bytes(b"S" * 4096)
...
def test_failed_geofiles_check_is_visible(panel, tmp_path: Path, monkeypatch):
    ...
    geoip.write_bytes(b"x" * 4096)
    geosite.write_bytes(b"y" * 4096)
...
def test_advanced_and_geofiles_pages_contain_explanations_and_examples(panel):
    ...
    for text in (
        "Эффективные параметры текущей схемы",
        "XHTTP Mode",
        "Server Extra",
        "Client Extra",
        "FinalMask",
        "Certificate pinning",
        "Показать пример XHTTP Extra",
        "Показать пример FinalMask",
        "Проверка сертификата клиента",
        "CA PEM / полная цепочка",
        "TLS завершается в Nginx",
    ):
        assert text in body
    ...
    body = response.get_data(as_text=True)
    for text in (
        "V2Fly", "Loyalsoldier", "RunetFreedom",
...
def test_release_contains_rc52_contextual_documentation():
    ...
    assert "ДОПОЛНИТЕЛЬНО" in advanced
    assert "Фактически используемые geoip.dat" in geofiles
    assert "SG-Panel 054 — guided Cluster onboarding and Firefox REALITY defaults" in css
    assert "не записывается автоматически" in advanced.lower()
